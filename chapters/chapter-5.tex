...
module.exports = function () {
    ...
    this.Given(/^I Correctly type in my details$/, {timeout: 60*1000}, function () {
...
In \textbf{Listing \ref{lst:example-page-file}}, you can see that we create a map of ll the elements of the payment form. Now we can write code to carry out intercations on the page. The first step definition of clicking some button on some page is pretty generic and following the conventions of our framework metioned in \ref{subsec:reusable-support-functions}, we should implement a generic method to handle this interaction in the support.js file.
...
go like we start with a requirement -> adding a feature file -> writing a full test in Gherkin and let it fail -> then developers start developing -> at the same time tests write steps -> add pages -> ...
