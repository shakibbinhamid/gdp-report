...
\begin{figure}[!hbt]
  	\centering
 	\includegraphics[width=0.3\textwidth]{synote-screenshot.png}
...
Synote wishes to be flexible with the various sites where it operates. This allows for variation of the functionality according to the needs of each client. This also entails having several different deployments and managing them. In order to better prepare for the production environment, the app is first deployed on other environments and tested. \\

One such environment is \url{http://www.experiment.synote.com}. It can be found on an AWS linux machine running Nginx as its web server container. As the name suggests, it is primarily used for experimenting with new features, without disrupting the production environment. The experiment deployment is the first step in making sure the application is ready to be released to users after any new changes are made. \\
