...
When charging against saved card, we invoke the same method from “paymentservice” used for charging against a new card but with different parameters. Consider \textbf{Listing \ref{lst:params-charge-card}}:\\
...
//code omitted   
\end{minted}
\captionof{listing}{Section of buycredit.js file}
\label{lst:params-charge-card}
\end{listing} 

Instead of passing in the Stripe token, we now pass the “cardId” of selected card.\\ 

\subsubsection{Credits History Feature}
\label{subsubsec:credits-history-feature}

When user is routed to the “creditsHistory” URI, we send an HTTP Get request to Synote’s server to retrieve credits history for logged in user. Consider \textbf{Listing \ref{lst:credits-history-req}}:\\
...
The loaded credit history data then displayed as a table with most recent purchases first.\\ 

\subsection{Clientside Payment Details Validation}
\label{subsec:clientside-payment-details-validation}

Client side validation is handled via methods provided by Stipe’s API and an angular payment validation module (see top connection fig) called “angular-payments” [3]. Stripe’s validation methods are called via “buycredit.html” page &>& “buycredit” controller &>& “paymentservice” service.\\
...
In \textbf{Listing \ref{lst:buycredit-html-cardnumber-input}}, we  use Angluar’s ng-change directive to execute “checkCardNumber” method whenever it detects an input change e.g. keypress. “payments-format=”card”” is provided by “angular-payments” and ensures input is maximum of 16 digits.\\
...
In code snippet \textbf{Listing \ref{lst:buycredit-controller-checkcardnumber-method}}, we set validity of form based on the input card number via \texttt{paymentService}.\\
