...
\begin{figure}[!hbt]
  	\centering
 	\includegraphics[width=0.7\textwidth]{live-validation-screenshot.png}
...
Synote uses \texttt{Toastr} alert messages throughout its implementation. We hit a problem when writing automated E2E tests where the \texttt{Toastr} alerts would prevent WebDriver from accessing the logout button. Consider \textbf{Figure \ref{fig:toastr-alert-problem-screenshot}}:\\
...
\begin{minted}[xleftmargin=\parindent, linenos, breaklines, breakanywhere, bgcolor=lightgray, fontsize=\small]{bash}
//code omitted    
<uib-alert dismiss-on-timeout="5000" ng-repeat="alert in alerts|limitTo:4" type="{{alert.type}}"
       close="closeAlert($index)"><strong>{{ alert.title }}</strong> {{ alert.msg }}
</uib-alert>
//code omitted   
\end{minted}
\captionof{listing}{Section of \texttt{buycredit.html} file}
...
\subsubsection{Screenshots of Alerts in Action}
\label{subsubsec:screenshots-alerts}
...
To fit in with the responsive style of Synote, Bootstrap framework was used and the responsiveness of view files we added were tested using Firefox’s responsive design mode [5] which let us emulate screen sizes of different devices. 

Consider the screenshots of tests in action:\\

\begin{figure}[!hbt]
  	\centering
 	\includegraphics[width=0.5\textwidth]{responsive-profile-page.png}
...
\begin{figure}[!hbt]
  	\centering
 	\includegraphics[width=0.6\textwidth]{responsive-credits-history.png}
...
\begin{figure}[!hbt]
  	\centering
 	\includegraphics[width=0.5\textwidth]{responsive-payment-form.png}
...
\begin{figure}[!hbt]
  	\centering
 	\includegraphics[width=0.6\textwidth]{responsive-saved-form.png}
